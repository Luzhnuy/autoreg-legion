...
def mainth():
	while True:
		try:
			...
			r = session.post('https://www.1secmail.com/api/v1/?action=genRandomMailbox&count=1')
			print('TEMP MAIL -------------')
			print(r.text)
			email = loads(r.text)[0]
			checkmailsurl = 'https://www.1secmail.com/api/v1/?action=getMessages&login=demo&domain=' + email
